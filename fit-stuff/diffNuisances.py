...
parser.add_option("-o", "--only-signal", action='store_true', help="If true, plot the pulls only for s+b fit.")
...
if options.plotfile:
    ...
    if not options.only_signal: gr_fit_b.Draw("EPsame")
    ...
    if not options.only_signal: leg.AddEntry(gr_fit_b,"B-only fit","EPL")
    ...
    if not options.only_signal: leg_rat.AddEntry(hist_fit_e_b,"B-only fit","F")
